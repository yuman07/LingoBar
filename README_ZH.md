...
Swift 6.3 已经随 Xcode 26 一同装好，不用单独装 Swift toolchain。SwiftPM 依赖（KeyboardShortcuts）会在第一次用 Xcode 打开工程时自动解析。
...
# 2. 用 Xcode 打开 — 第一次打开时 SwiftPM 会自动拉 KeyboardShortcuts
...
- **外部服务** — `KeyboardShortcuts` 拥有全局快捷键和 `StatusBarController` 的双向同步；Apple `Translation` 框架在端上运行，遇到"语言包未安装"也按普通引擎失败处理由下一个引擎接手。
...
|-- AppDelegate.swift            # NSApplicationDelegate，全局接线
